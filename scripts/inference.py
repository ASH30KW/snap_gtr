...
def render_mesh_to_gif(
    ...
    rgbs_raw = out.cpu().numpy()[:, ::-1, :, :]
    alpha = rgbs_raw[..., 3:]

    if transparent:
        # Keep RGBA for transparent output
        rgbs = np.concatenate([rgbs_raw[..., :3], alpha], axis=-1)
        rgbs = (rgbs * 255.0).astype(np.uint8)
    else:
        # Composite with white background
        rgbs = rgbs_raw[..., :3] * alpha + np.ones(3) * (1 - alpha)
        rgbs = (rgbs * 255.0).astype(np.uint8)
    ...
    if transparent:
        # Keep RGBA for transparent output
        normal_img = np.concatenate([normal_img[..., :3], alpha], axis=-1)
    ...
    if transparent:
        # For transparent output, only save mesh render (no normal map side-by-side)
        imgs = rgbs
    ...
    save_images_to_gif(imgs, output_file=gif_file, transparent=transparent)


def load_eval_data(in_dir, input_img_res=512, radius=1.0, num_images=None, image_indices=None):
...
@click.option("--num_images", type=int, default=None, help="number of input images to use (default: all)")
@click.option("--image_indices", type=str, default=None, help="comma-separated indices of images to use (e.g., '0,2,4,5' for 1st,3rd,5th,6th)")
@click.option("--skip_mesh_render", is_flag=True, default=False, help="Skip mesh GIF rendering to save time")
def main(**kwargs):
    opt = EasyDict(kwargs)
    ...
    image_indices = opt.image_indices
    if image_indices is not None:
        image_indices = [int(x) for x in image_indices.split(',')]
    print(f"---------------options-----------------")
    ...
    # load data
    data_batch = load_eval_data(in_dir, num_images=num_images, image_indices=image_indices)
...
@click.option("--render_mesh_res", type=int, default=512, help="resolution of mesh renering")
@click.option("--render_nerf_res", type=int, default=1024, help="resolution of NeRF renering")
@click.option("--skip_mesh_export", is_flag=True, default=False, help="Skip mesh extraction (no OBJ/GLB files)")
@click.option("--skip_mesh_gif", is_flag=True, default=False, help="Skip mesh GIF rendering (faster, still exports mesh files)")
@click.option("--render_num_frames", type=int, default=50, help="Number of frames for GIF rendering")
@click.option("--render_fov", type=float, default=50, help="FOV for rendering (degrees)")
@click.option("--render_cam_distance", type=float, default=3.5, help="Camera distance for rendering")
@click.option("--render_elevation", type=float, default=20, help="Camera elevation for rendering (degrees)")
@click.option("--export_glb", is_flag=True, default=False, help="Also export mesh in GLB format")
@click.option("--transparent", is_flag=True, default=False, help="Render GIF with transparent background")
def main(**kwargs):
    opt = EasyDict(kwargs)
    config = "configs/config_texrefine.yaml"
    job_num = 0
    ...
    # set up cameras for mesh rendering (use command line options)
    ...
    # Add alpha channel for transparent background if requested
    if opt.transparent and 'antialias_mask' in outputs:
        alpha = outputs['antialias_mask'][0]
        alpha = (alpha.cpu().numpy() * 255.0).astype(np.uint8)[:, ::-1, :, :]
        # Expand alpha to match RGB if needed (it might be single channel)
        if alpha.shape[-1] == 1:
            pass  # Already single channel
        elif len(alpha.shape) == 3:
            alpha = alpha[..., np.newaxis]
        rgbs = np.concatenate([rgbs, alpha], axis=-1)  # RGBA
        logger.info("Using antialias_mask as alpha for transparent background")

    nerf_rgb_gif_file = f"{out_dir}/nerf.gif"
    save_images_to_gif(rgbs, output_file=nerf_rgb_gif_file, transparent=opt.transparent)
    logger.info(f"save NeRF rgb rendering to {out_dir} (transparent={opt.transparent})")

    # mesh extraction (skip if --skip_mesh_export is set)
    if not opt.skip_mesh_export:
        ...
        logger.info(f"Saved OBJ mesh to {mesh_file_obj}")

        # Also export GLB if requested
        if opt.export_glb:
            mesh_file_glb = f"{out_dir}/mesh.glb"
            mesh_list[0].export(mesh_file_glb)
            logger.info(f"Saved GLB mesh to {mesh_file_glb}")

        # render mesh GIF (skip if --skip_mesh_gif is set)
        if not opt.skip_mesh_gif:
            ...
            logger.info(f"Save mesh GIF to {mesh_gif_file} (transparent={opt.transparent})")
        else:
            logger.info("Skipping mesh GIF rendering (--skip_mesh_gif flag set)")
    else:
        logger.info("Skipping mesh extraction (--skip_mesh_export flag set)")
